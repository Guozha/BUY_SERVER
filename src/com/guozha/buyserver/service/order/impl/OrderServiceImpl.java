...
public class OrderServiceImpl extends AbstractBusinessObjectServiceMgr implements OrderService {
	...
	public MsgResponse cancelOrder(CancelOrderRequest vo) {
		buyOrderMapper.updateStatus(vo.getOrderId(), vo.getOldStatus(), "00"); // ORDER_STATUS
		// 00-取消
	...
	public MsgResponse insertOrder(InsertOrderRequest vo) {
		...
		buyOrder.setOrderNo(commonService.getPaperNo(SystemResource.getConfig("area_code.hangzhou"), "01"));// PAPER_TYPE
		// 01-用户订单
		...
		for (BuyCart buyCart : buyCartList) {
			...
			if ("01".equals(buyCart.getSplitType()))
				continue; // 过滤掉菜谱 SPLIT_TYPE 01-菜谱
		...
		for (BuyCart buyCart : buyCartList) {
			...
			if ("02".equals(buyCart.getSplitType()))
				continue; // 过滤掉食材 SPLIT_TYPE 02-食材
	...
	public void sendOrder(int orderId) {
		...
		for (BuyOrderChild buyOrderChild : buyOrderChildList) {
			...
			if (sysSellerList.size() == 1) {
				...
				buyOrderChildMapper.updateBySend(buyOrderChild.getOrderChildId(), sysSeller.getSellerId(), sysSeller.getSellerId(), "03"); // ORDER_CHILD
				// 03-已抢单
			} else {
	 ...
	 * @return MsgResponse
	 */
	public MsgResponse markOrderMenuGoods(MarkRequest mark) {
		MsgResponse response = new MsgResponse();
		if (mark != null) {
			Integer orderMenuGoodsId = mark.getOrderGoodsId();
			if (orderMenuGoodsId != null) {
				// 更新订单菜谱商品表
				BuyOrderMenuGoods orderMenuGoods = new BuyOrderMenuGoods();
				orderMenuGoods.setOrderMenuGoodsId(orderMenuGoodsId);
				orderMenuGoods.setGoodsStar(mark.getGoodsStar());
				int count = buyOrderMenuGoodsMapper.markOrderMenuGoods(orderMenuGoodsId);
				if (count != 1) {
					response.setReturnCode(YesNo.No.getCode().toString());
					response.setMsg("更新错误");
				}
			}
		}
		return response;
	 ...
	 * @return
	 */
	public MsgResponse markOrderGoods(MarkRequest mark) {
		MsgResponse response = new MsgResponse();
		if (mark != null) {
			Integer orderGoodsId = mark.getOrderGoodsId();
			if (orderGoodsId != null) {
				// 更新订单商品表
				BuyOrderGoods buyOrderGoods = new BuyOrderGoods();
				buyOrderGoods.setOrderGoodsId(orderGoodsId);
				buyOrderGoods.setGoodStar(mark.getGoodsStar());
				int count = buyOrderGoodsMapper.markOrderGoods(buyOrderGoods);
				if (count != 1) {
					response.setReturnCode(YesNo.No.getCode().toString());
					response.setMsg("更新错误");
				}
	 ...
	 * @return
	 */
	public MsgResponse goodsMark(MarkRequest mark) {
		MsgResponse response = null;
		if (mark != null) {
			String markType = mark.getMarkType();
			if ("1".equals(markType))
				response = markOrderMenuGoods(mark);
			else if ("2".equals(markType))
				response = markOrderGoods(mark);
		}
		return response;
	}
	 ...
	 * @return
	 */
	public MsgResponse orderMark(MarkRequest mark) {
		MsgResponse response = new MsgResponse();
		if (mark != null) {
			BuyOrder buyOrder = new BuyOrder();
			buyOrder.setOrderId(mark.getOrderId());
			buyOrder.setCommentDesc(mark.getCommentDesc());
			buyOrder.setCommentFlag(YesNo.Yes.getCode().toString());
			buyOrder.setServiceStar(mark.getServiceStar());
			int count = buyOrderMapper.orderMark(buyOrder);
			if (count != 1) {
				response.setReturnCode(YesNo.No.getCode().toString());
				response.setMsg("更新错误");
			}
