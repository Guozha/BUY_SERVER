...
import com.guozha.buyserver.persistence.mapper.BuyCartMappers;
...
import com.guozha.buyserver.service.cart.CartServices;
...
@Service("cartService")
public class CartServiceImpls extends AbstractBusinessObjectServiceMgr implements CartServices {
    ...
    @Autowired
	private BuyCartMappers buyCartMappers;
	...
	public MsgResponse add(CartRequest vo) {
		...
		BuyCart po = this.buyCartMappers.selectByGoodsOrMenuId(vo.getUserId(), vo.getId(),vo.getProductType());
		if(po==null){  //新的购物车信息
			po = new BuyCart();
			...
			this.buyCartMappers.insert(po);
		}else{  //修改购物车信息
			po.setAmount(vo.getAmount()[0]);
			this.buyCartMappers.update(po);
		}
	...
	public MsgResponse update(CartRequest vo) {
		...
		//删除已被用户移除的购物车信息
		this.buyCartMappers.deletesNotIn(vo.getUserId(), vo.getCartId());
		return new MsgResponse(MsgResponse.SUCC, "修改购物车成功");
	}

	@Override
	public MsgResponse del(CartRequest vo) {
		this.buyCartMappers.deletes(vo.getCartId());
		return new MsgResponse(MsgResponse.SUCC, "删除购物车成功");
	}

	@Override
	public MsgResponse clear(CartRequest vo) {
		this.buyCartMappers.deleteByUserId(vo.getUserId());
		return new MsgResponse(MsgResponse.SUCC, "清空购物车成功");
	}
    
	
	@Override
	public CartResponses find(CartRequest vo) {
       
		CartResponses response = new CartResponses();
		...
		//菜谱
		List<BuyCart> menuCartList = this.buyCartMappers.findMenuByUserId(vo.getUserId());
		...
		//食材
		List<BuyCart> goodsCartList = this.buyCartMappers.findGoodsByUserId(vo.getUserId());
