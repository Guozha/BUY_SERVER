...
public class SolarTermsUtil {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("农历" + l.getSolarTermsUtilDateString());
    ...
    public static long getbeforesolarTerm(int year, Date date) {
        Date[] jieqi = jieqilist(year);
        int[] jieqibeforeafter = getnearsolarTerm(year, date);

        return SolarTermsUtils.DateDays(date, jieqi[jieqibeforeafter[0]]);
    ...
    public static long getaftersolarTerm(int year, Date date) {
        ...
        return SolarTermsUtils.DateDays(date, jieqi[jieqibeforeafter[1]]);
    ...
    public static int[] getnearsolarTerm(int year, Date date) {
        ...
        for (int i = 0; i < jieqi.length; i++) {
            if (date.getTime() > jieqi[i].getTime()) {
                continue;
            }
            returnValue[0] = i - 1;
            returnValue[1] = i;
    ...
    private final static int[] SolarTermsUtilInfo = {
    ...
    public final static String[] SolarTermsUtilString1 = {
        "零", "一", "二", "三", "四", "五", "六", "七", "八", "九"
    };
    public final static String[] SolarTermsUtilString2 = {
        "初", "十", "廿", "卅", "正", "腊", "冬", "闰"
    };
    /**
     * 国历节日 *表示放假日
    ...
    private synchronized void findFestival() {
        ...
        int lM = this.getSolarTermsUtilMonth();
        int lD = this.getSolarTermsUtilDay();
        int sy = this.getSolarYear();
        Matcher m;
        for (int i = 0; i < SolarTermsUtil.sFtv.length; i++) {
            m = SolarTermsUtil.sFreg.matcher(SolarTermsUtil.sFtv[i]);
            if (m.find()) {
        ...
        for (int i = 0; i < SolarTermsUtil.lFtv.length; i++) {
            m = SolarTermsUtil.sFreg.matcher(SolarTermsUtil.lFtv[i]);
            if (m.find()) {
        ...
        // 月周节日   
        int w, d;
        for (int i = 0; i < SolarTermsUtil.wFtv.length; i++) {
            m = SolarTermsUtil.wFreg.matcher(SolarTermsUtil.wFtv[i]);
            if (m.find()) {
     ...
     * 返回农历年闰月月份
     *
     * @param SolarTermsUtilYear 指定农历年份(数字)
     * @return 该农历年闰月的月份(数字,没闰返回0)
     */
    private static int getSolarTermsUtilLeapMonth(int SolarTermsUtilYear) {
        // 数据表中,每个农历年用16bit来表示,   
        // 前12bit分别表示12个月份的大小月,最后4bit表示闰月   
        // 若4bit全为1或全为0,表示没闰, 否则4bit的值为闰月月份   
        int leapMonth = SolarTermsUtil.SolarTermsUtilInfo[SolarTermsUtilYear - 1900] & 0xf;
     ...
     * @return 该农历年闰月的天数(数字)
     */
    private static int getSolarTermsUtilLeapDays(int SolarTermsUtilYear) {
        // 下一年最后4bit为1111,返回30(大月)   
        // 下一年最后4bit不为1111,返回29(小月)   
        // 若该年没有闰月,返回0   
        return SolarTermsUtil.getSolarTermsUtilLeapMonth(SolarTermsUtilYear) > 0 ? ((SolarTermsUtil.SolarTermsUtilInfo[SolarTermsUtilYear - 1899] & 0xf) == 0xf ? 30
                : 29)
                : 0;
    }
     ...
     * @return 该农历年的总天数(数字)
     */
    private static int getSolarTermsUtilYearDays(int SolarTermsUtilYear) {
        // 按小月计算,农历年最少有12 * 29 = 348天   
        int daysInSolarTermsUtilYear = 348;
        // 数据表中,每个农历年用16bit来表示,   
        // 前12bit分别表示12个月份的大小月,最后4bit表示闰月   
        // 每个大月累加一天   
        for (int i = 0x8000; i > 0x8; i >>= 1) {
            daysInSolarTermsUtilYear += ((SolarTermsUtil.SolarTermsUtilInfo[SolarTermsUtilYear - 1900] & i) != 0) ? 1
                    : 0;
        }
        // 加上闰月天数   
        daysInSolarTermsUtilYear += SolarTermsUtil.getSolarTermsUtilLeapDays(SolarTermsUtilYear);

        return daysInSolarTermsUtilYear;
    }
     ...
     *
     * @param SolarTermsUtilYear 指定农历年份(数字)
     * @param SolarTermsUtilMonth 指定农历月份(数字)
     * @return 该农历年闰月的月份(数字,没闰返回0)
     */
    private static int getSolarTermsUtilMonthDays(int SolarTermsUtilYear, int SolarTermsUtilMonth) {
        // 数据表中,每个农历年用16bit来表示,   
        // 前12bit分别表示12个月份的大小月,最后4bit表示闰月   
        int daysInSolarTermsUtilMonth = ((SolarTermsUtil.SolarTermsUtilInfo[SolarTermsUtilYear - 1900] & (0x10000 >> SolarTermsUtilMonth)) != 0) ? 30
                : 29;
        return daysInSolarTermsUtilMonth;
    }
    ...
    private int SolarTermsUtilYear;
    private int SolarTermsUtilMonth;
    private int SolarTermsUtilDay;
    ...
    private void init(long TimeInMillis) {
        ...
        this.SolarTermsUtilYear = 1900;
        int daysInSolarTermsUtilYear = SolarTermsUtil.getSolarTermsUtilYearDays(this.SolarTermsUtilYear);
        while (this.SolarTermsUtilYear < 2100 && offset >= daysInSolarTermsUtilYear) {
            offset -= daysInSolarTermsUtilYear;
            daysInSolarTermsUtilYear = SolarTermsUtil.getSolarTermsUtilYearDays(++this.SolarTermsUtilYear);
        }
        // 农历年数字   

        // 按农历月递减每月的农历天数，确定农历月份   
        int SolarTermsUtilMonth = 1;
        // 所在农历年闰哪个月,若没有返回0   
        int leapMonth = SolarTermsUtil.getSolarTermsUtilLeapMonth(this.SolarTermsUtilYear);
        // 是否闰年   
        this.isLeapYear = leapMonth > 0;
        // 闰月是否递减   
        boolean leapDec = false;
        boolean isLeap = false;
        int daysInSolarTermsUtilMonth = 0;
        while (SolarTermsUtilMonth < 13 && offset > 0) {
            if (isLeap && leapDec) { // 如果是闰年,并且是闰月   
                // 所在农历年闰月的天数   
                daysInSolarTermsUtilMonth = SolarTermsUtil.getSolarTermsUtilLeapDays(this.SolarTermsUtilYear);
                leapDec = false;
            } else {
                // 所在农历年指定月的天数   
                daysInSolarTermsUtilMonth = SolarTermsUtil.getSolarTermsUtilMonthDays(this.SolarTermsUtilYear, SolarTermsUtilMonth);
            }
            if (offset < daysInSolarTermsUtilMonth) {
                break;
            }
            offset -= daysInSolarTermsUtilMonth;

            if (leapMonth == SolarTermsUtilMonth && isLeap == false) {
                // 下个月是闰月   
                leapDec = true;
                isLeap = true;
            } else {
        ...
        this.maxDayInMonth = daysInSolarTermsUtilMonth;
        // 农历月数字   
        this.SolarTermsUtilMonth = SolarTermsUtilMonth;
        // 是否闰月   
        this.isLeap = (SolarTermsUtilMonth == leapMonth && isLeap);
        // 农历日数字   
        this.SolarTermsUtilDay = (int) offset + 1;
        // 取得干支历   
    ...
    public String getAnimalString() {
        return SolarTermsUtil.Animals[(this.SolarTermsUtilYear - 4) % 12];
    }
     ...
     * @return 农历日期字符串
     */
    public String getSolarTermsUtilDayString() {
        return SolarTermsUtil.getSolarTermsUtilDayString(this.SolarTermsUtilDay);
    }
     ...
     * @return 农历日期字符串
     */
    public String getSolarTermsUtilMonthString() {
        return (this.isLeap() ? "闰" : "") + SolarTermsUtil.getSolarTermsUtilMonthString(this.SolarTermsUtilMonth);
    }
     ...
     * @return 农历日期字符串
     */
    public String getSolarTermsUtilYearString() {
        return SolarTermsUtil.getSolarTermsUtilYearString(this.SolarTermsUtilYear);
    }
     ...
     */
    public String getSolarTermsUtilDateString() {
        return this.getSolarTermsUtilYearString() + "年"
                + this.getSolarTermsUtilMonthString() + "月"
                + this.getSolarTermsUtilDayString() + "日";
    }
     ...
     * @return 农历日期
     */
    public int getSolarTermsUtilDay() {
        return SolarTermsUtilDay;
    }
     ...
     * @return 农历月份
     */
    public int getSolarTermsUtilMonth() {
        return SolarTermsUtilMonth;
    }
     ...
     * @return 农历年份
     */
    public int getSolarTermsUtilYear() {
        return SolarTermsUtilYear;
    }
     ...
     * 返回指定数字的农历年份表示字符串
     *
     * @param SolarTermsUtilYear 农历年份(数字,0为甲子)
     * @return 农历年份字符串
     */
    private static String getSolarTermsUtilYearString(int SolarTermsUtilYear) {
        return SolarTermsUtil.getCyclicalString(SolarTermsUtilYear - 1900 + 36);
    }

    /**
     * 返回指定数字的农历月份表示字符串
     *
     * @param SolarTermsUtilMonth 农历月份(数字)
     * @return 农历月份字符串 (例:正)
     */
    private static String getSolarTermsUtilMonthString(int SolarTermsUtilMonth) {
        String SolarTermsUtilMonthString = "";
        if (SolarTermsUtilMonth == 1) {
            SolarTermsUtilMonthString = SolarTermsUtil.SolarTermsUtilString2[4];
        } else {
            if (SolarTermsUtilMonth > 9) {
                SolarTermsUtilMonthString += SolarTermsUtil.SolarTermsUtilString2[1];
            }
            if (SolarTermsUtilMonth % 10 > 0) {
                SolarTermsUtilMonthString += SolarTermsUtil.SolarTermsUtilString1[SolarTermsUtilMonth % 10];
            }
        }
        return SolarTermsUtilMonthString;
    }

    /**
     * 返回指定数字的农历日表示字符串
     *
     * @param SolarTermsUtilDay 农历日(数字)
     * @return 农历日字符串 (例: 廿一)
     */
    private static String getSolarTermsUtilDayString(int SolarTermsUtilDay) {
        if (SolarTermsUtilDay < 1 || SolarTermsUtilDay > 30) {
            return "";
        }
        int i1 = SolarTermsUtilDay / 10;
        int i2 = SolarTermsUtilDay % 10;
        String c1 = SolarTermsUtil.SolarTermsUtilString2[i1];
        String c2 = SolarTermsUtil.SolarTermsUtilString1[i2];
        if (SolarTermsUtilDay < 11) {
            c1 = SolarTermsUtil.SolarTermsUtilString2[0];
        }
        if (i2 == 0) {
            c2 = SolarTermsUtil.SolarTermsUtilString2[1];
        }
        return c1 + c2;
    }
}
